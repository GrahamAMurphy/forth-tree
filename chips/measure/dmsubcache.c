...
#define SUBMISS(x)\
	 ((valid[INDEX(x)] & (1<<SUBINDEX(x))) == 0)
#define ALLOCLINE(x)\
	tagram[INDEX(x)] = (x) & tagmask;\
	valid[INDEX(x)] = 0
#define ADDTOLINE(x)\
	valid[INDEX(x)] |= 1<<SUBINDEX(x)
...
unsigned int readupdate;
unsigned int externalwrite;
unsigned int writeupdate;
...
void SimulateMixedMissAllocV2();
void SimulateMixedNeverAlloc();
void SimulateMixedNeverAllocV2();
void Summarize();
...
void
SimulateMixedNeverAllocV2(fp,remainingaccess)
FILE *fp;
unsigned int remainingaccess;
{
	unsigned int trace, x;
...
int verbose;
{
	if(verbose){
		printf("%d:%d:%d:%d:%d\n",
			cpucycle, externalread, readupdate,
			externalwrite, writeupdate);
	} else {
		printf("%f\n",
			(float)miss /
			(float)cpucycle);
