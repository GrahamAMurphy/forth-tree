...
static branchtaken;			/* count conditional branches taken */
static fillable;			/* conditional branch is fillable */
...
Execphib()
{
	...
	/* if last instruction is ?branch and branch is taken */
	if((oldir.irbranch.type == B_QBRANCH) && (flag == 0)){
		branchtaken++;
		/* if it is not a branch and not a return */
		if((ir.irbranch.type > 3) && (ir.irload.ret != B_RETURN)){
			fillable++;
		}
	}
	phase = 1;
...
Summary()
{
	...
	printf("\t%9d %s (%d taken, %d filled)\n",
		instrtypes[2],instrnames[2],branchtaken,fillable);
	for(i=3; i<8; i++){
		printf("\t%9d %s\n",instrtypes[i],instrnames[i]);
	}
	printf("cycle types:\n");
