...
#include "globals.h"
#include "external.h"
#include "sc32internal.h"
#include "sc32formats.h"
#include "sc32masks.h"
...
static filledslots;			/* filled call slots */
static loadindependent;			/* load does not need to stall */
...
Loadphib()
{
	...
	if(((ir.irbranch.type == B_MICRO) &&
	    (oldir.irload.reg2 == B_TOSR2))
	|| ((ir.irbranch.type > B_QBRANCH) &&
	    (oldir.irload.reg2 == ir.irload.reg1))){
		cycletypes[T_EXEC]++;	/* else add cycle */
		cycle++;
	} else {
		loadindependent++;
	}
...
Execphia()
{
	abus = Asrca() & ~0x10000000;	/* ignore call slot bit */
	portio = PORT_RD;
	alub = bbus = Bsrca();
	alua = tbus = Tsrca();
	portaddr = abus;
...
Execphib()
{
	...
	if(ir.irbranch.type == B_COLON){	/* if call */
		if(ir.irbranch.dest & 0x10000000){ /* if slot not filled */
			cycletypes[T_EXEC]++;	/* else add cycle */
			cycle++;
		} else {
			filledslots++;
...
Summary()
{
	...
	printf("\t%9d %s (%d filled)\n",
		instrtypes[0],instrnames[0],filledslots);
	for(i=1; i<4; i++){
		printf("\t%9d %s\n",instrtypes[i],instrnames[i]);
	}
	printf("\t%9d %s (%d independent)\n",
		instrtypes[4],instrnames[4],loadindependent);
	for(i=5; i<8; i++){
		printf("\t%9d %s\n",instrtypes[i],instrnames[i]);
	}
	printf("cycle types:\n");
