...
#define BRADDR(x)   ((x)<<2)		/* extract compressed branch address */
...
static unsigned int lowtop = 0x40000;	/* maximum memory address (default) */
...
FILE *fp;
{
	...
	fstat(fileno(fp),&buf);
	if((unsigned int)buf.st_size > lowtop){
		fprintf(stderr,"Too big; use -r to allocate more memory\n");
		exit(1);
	}
	if((lowmem=(unsigned char *)malloc(lowtop)) == NULL){
...
FILE *fp;
{
	...
	if((highmem=(unsigned char *)malloc((int)4*buf.st_size)) == NULL){
		fprintf(stderr,"Malloc failed\n");
		return;
	}
...
{
	if((unsigned)addr <= lowtop){
		*(forthword *)(lowmem + TRUNCATE(addr)) = value;
	} else if((unsigned)addr > TRAPSPACE){
		Fakeioarg(value);
	} else if(((unsigned)addr >= highbottom) &&
		  ((unsigned)addr <= hightop)){
		*(forthword *)(highmem + TRUNCATE(addr) - highbottom) = value;
	} else if(psw.fields.ibrenable &&
		  ((unsigned)addr >= ibrombottom) &&
		  ((unsigned)addr <= ibromtop)){
...
forthword value;
{
	if((unsigned)addr <= lowtop){
		...
		*(unsigned char *)(highmem + addr - highbottom) =
		...
		Iowrite(addr,value);  /*TBD Iowritebyte? */
	}
}
	...
	switch(state){
	   ...
	   case T_EXEC:
		if(ira.irbranch.type == B_LOAD){
			state = T_LOAD;
		} else if(ira.irbranch.type == B_LOADBYTE){
			state = T_LOAD;
		} else if(ira.irbranch.type == B_STORE){
			state = T_STORE;
		} else if(ira.irbranch.type == B_STOREBYTE){
			state = T_STORE;
...
Loadphia()
{
	...
	if(oldir.irload.type == B_LOADBYTE) portio = PORT_RDBYTE;
	else				    portio = PORT_RD;
	phase = 2;
}
...
Loadphib()
{
	...
	if(oldir.irload.type == B_LOADBYTE){
		switch(BYTESEL(portaddr)){	/* big-endian */
		   case 0:	dhold = (portvalue>>24) & 0xff; break;
		   case 1:	dhold = (portvalue>>16) & 0xff; break;
		   case 2:	dhold = (portvalue>>8)  & 0xff; break;
		   case 3:	dhold = (portvalue)     & 0xff; break;
		}
	} else {
		dhold = portvalue;
	}
...
{
	portaddr = abus = aluout;
	if(oldir.irload.type == B_STOREBYTE) portio = PORT_WRBYTE;
	else				     portio = PORT_WR;
	phase = 2;
}
...
Storephib()
{
	bbus = Readreg(ira.irload.reg2);
	if(oldir.irload.type == B_STOREBYTE){
		switch(BYTESEL(portaddr)){	/* big-endian */
		   case 0:	portvalue = (bbus<<24); break;
		   case 1:	portvalue = (bbus<<16); break;
		   case 2:	portvalue = (bbus<<8) ; break;
		   case 3:	portvalue = (bbus)    ; break;
		}
		Storeexternalbyte(portaddr, bbus&0xff);
	} else {
		portvalue = bbus;
...
Asrca()
{
	...
	switch(irb.irbranch.type){
	   case B_FLOW:
		switch(irb.irbranch.brtype){
		   case B_COLON:
		   case B_BRANCH:
			result = BRADDR(irb.irbranch.dest);
			break;
		   case B_QBRANCH:
			result = flag ? pcb : BRADDR(irb.irbranch.dest);
			break;
		   default:
...
Stackop()
{
	switch(ira.irbranch.type){
	   case B_FLOW:
		if(ira.irbranch.brtype == B_COLON){
