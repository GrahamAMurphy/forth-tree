...
def fMemory(oSeq):
  ...
  fSendCmd('STAT_MEM', 0x60000)
  fGoodCmd()
  fCheckEcho(oSeq, 'STAT_MEM', 0, [0x00,0x06,0x00,0x00])
  fCheckPacket(oSeq, bBootStatus, lambda: (bBootStatus.WatchMemId==0x6) and
  				(bBootStatus.WatchAddr==0x0000))
  fCheckCmdCounts(oSeq)
  C('---MEM_LOAD should be rejected; write disabled---')
  if host=='MESS':			# MESSENGER GSEOS requires 'type'
    fSendCmd('MEM_LOAD', 0x60000, 0, 1, 2, 3, 4) # TBD: work around GSEOS bug
  else:
    fSendCmd('MEM_LOAD', 0x60000, 1, 2, 3, 4) # TBD: work around GSEOS bug
  fBadCmd()
  fCheckEcho(oSeq, 'MEM_LOAD', 0x0a,
		[0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x01, 0x02])
  fCheckCmdCounts(oSeq)
  C('---MEM_COPY should be rejected; write disabled---')
  fSendCmd('MEM_COPY', 0, 0x60000, 16)
  fBadCmd()
...
#		[0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10])
  ...
  fCheckEcho(oSeq, 'STAT_INT', 0, [0,0])
  if host=='MESS':			# MESSENGER GSEOS requires 'type'
    fSendCmd('MEM_READ', 0x60000, 4096, 0)
  else:
    fSendCmd('MEM_READ', 0x60000, 4096)
  fGoodCmd()
  fCheckEcho(oSeq, 'MEM_READ', 0x00,
