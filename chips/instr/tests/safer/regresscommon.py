...
def fBasicCommand(oSeq):
  ...
  elif host=='MRO' or host=='Juno':	# MRO-specific
  ...
  fSendCmd(undefcmd)
  fBadCmd()
  if host=='MRO' or host=='Juno':	# MRO-specific; echoes 2-byte pad
    fCheckEcho(oSeq, undefcmd, 2, [0,0])
  else:
...
# TBD: missing test of bad EEPROM checksum
def fBoot(oSeq):
  # Precondition: status interval set to a high rate.
  ...
  fCheckCmdCounts(oSeq)
  C('---Load a file: %s---' % progname)
  fLoadFile(progname, 0)
  fSyncIdle(oSeq, 5)			# wait for command echos to stop
  fSyncCmd()				# sync up command counters
  C('---MEM_CHECK used to verify successful load---')
  fSendCmd('MEM_CHECK', 0, progwords)
  fGoodCmd()
# fCheckEcho(oSeq, 'MEM_CHECK', 0x00,	# there is a race between echo and check
#		[0x00, tstpg, 0x00, 0x00, 0x18, 0x06])
  fCheckMemCheck(oSeq, 0, progwords, progsum)
  fCheckCmdCounts(oSeq)
  ...
  fCheckPacket(oSeq, bBootStatus, lambda: (bBootStatus.MemWrite==1))
  fCheckCmdCounts(oSeq)
  C('---Load a file: %s' % eeprogname)
  fLoadFile(eeprogname, 0)
  fSyncIdle(oSeq, 5)			# wait for command echos to stop
  fSyncCmd()				# sync up command counters
  C('---MEM_CHECK used to verify successful load---')
  fSendCmd('MEM_CHECK', 0, eeprogwords)
  fGoodCmd()
# fCheckEcho(oSeq, 'MEM_CHECK', 0x00,	# there is a race between echo and check
#		[0x00, tstpg, 0x00, 0x00, 0x18, 0x06])
  fCheckMemCheck(oSeq, 0, eeprogwords, eeprogsum)
  fCheckCmdCounts(oSeq)
  ...
  oSeq.MessageBoxModeless('Confirm Forth application.  Type: 2 dpu-cfg! TBD')
