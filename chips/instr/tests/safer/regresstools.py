...
instr = 'JXX'; cmdprefix = instr; host = 'Juno'
...
if host == 'MESS':
  ...
  # Programs to use in boot program test
...
if host == 'Juno':
  ...
  from JEDICommands import fSetMacroMode, \
	fMemLoad, fWrapCommand, fLoadFile

  def fSetTime(t):
    ExecCmd('GSEOS_CMD_SET_TIME(%d, 0)' % t)

  # TBD: move to JEDICommands ?
  from types import IntType
  def CMD(name, args):
    if name=='JXX_CMD_WRAP':
      fWrapCommand(args[0], *args[1:])
    elif name=='JXX_MEM_LOAD':
